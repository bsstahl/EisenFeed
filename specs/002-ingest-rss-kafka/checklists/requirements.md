...
- US2 red-test checkpoint (Phase 3): `dotnet build tst/EisenFeed.Ingestion.Orchestration.Tests/EisenFeed.Ingestion.Orchestration.Tests.csproj` fails at compile-time with 8 errors. Missing production types: `IngestionOrchestrator`, `ITrackFeedItemIngestion`, `FeedItemIngestion`, `FeedItemIngestionStatus`. These must be created in EisenFeed.Ingestion.Orchestration and EisenFeed.Core during the GREEN phase.
